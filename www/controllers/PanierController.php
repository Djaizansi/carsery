...
foreach($allPage as $unePage){
    ...
    foreach($allShort as $unShort){
        ...
        foreach($verifCode as $unCode){
            if($unCode == $shortcode && $unShort->getType() == "piece"){
                $foundtest = true;
                break;
            }
        }
...
class PanierController {
    ...
    {
        if(Session::estConnecte() && ((Session::estClient() && FOUND) || Session::estAdmin())){
            $myView = new View("commandpiece",TEMPLATE);
 ...
 */
    ...
    public function addPieceToPanierAction(){
		...
		if(Session::estConnecte() && (Session::estClient() && FOUND)){
			...
			if(isset($_POST)){
				...
				if( ! $found ){
					...
					if( $nbPiecesPanierUser !=  3){
						...
						$exist = isset($_SESSION['uriPiece']) ? $_SESSION['uriPiece'] : Helpers::getUrl('myProject','view');
						Helpers::redirect($exist);
    ...
    public function consultPanierAction(){
		...
		if(Session::estClient()){
			if(FOUND){
				...
				$myView = new View('consultpanier',TEMPLATE);
				...
				throw new RouteException("Veuillez ajouter un shortcode de type 'Pièce' pour afficher le panier");
			}
