...
namespace carsery\core\Builder;
...
class QueryBuilder
{
    ...
    public function join( string $table, string $aliasTarget, string $fieldSource = 'id', string $fieldTarget = 'id' ): QueryBuilder
    {
        ...
        $this->addToQuery(" INNER JOIN $table $aliasTarget ON ". $aliasSource.".".$fieldSource." = " .$aliasTarget.".".$fieldTarget); //Jointure Full Join pour récupèrer des 2 tables
        return $this;
    }
    ...
    {
        $aliasSource = $this->alias;
        $this->addToQuery(" LEFT JOIN $table $aliasTarget ON ". $aliasSource.".".$fieldSource." = " .$aliasTarget.".".$fieldTarget); // Left Join pour récupèrer que la partie gauche des résultats et donc renvoyer Null a certaine partie
        return $this;
    }
